...
# BTI
file_path <- "data/BTI_2006-2024_Scores.xlsx"   # File path
sheets <- excel_sheets(file_path)  # Sheet names
read_bti_sheet <- function(sheet_name) {    # Robust sheet-reading function
  df <- read_excel(file_path, sheet = sheet_name)
  
  tibble(
    year = as.integer(str_remove(sheet_name, "BTI ")),
    country = df[[1]],
    bti_IGs = suppressWarnings(as.numeric(df[[26]]))  # force numeric with suppression of NA warnings
  )
}
bti <- map_dfr(sheets, read_bti_sheet)   # Read and combine
bti$ISO <- countrycode(bti$country , origin = 'country.name', destination = 'iso3c')
bti <- bti %>%      # Fill ISO for unmatched cases where country names match historical ones
...
df_names <- c("vdem", "qog", "bti", "clea_enp", "cst_counts", "eci", 
              "genepy", "gwf", "kailitz", "personalism", "wth", "hdi")
...
merge <- merge(merge, hdi, by = c("ISO", "year"),all.x = T, all.y = F)
