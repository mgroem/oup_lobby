...
vdem <- vdem[c("ISO", "year",
               "logpop", "cso_repress", "election", "v2pscohesv_osp", "v2x_feduni", "v2x_regime2", 
...
merge <- merge(vdem, qog, by = c("ISO", "year"),all.x = T, all.y = T)
...
test <- aut[c("ISO", "year", "gwf_regimetype4", "wth_regime", "kailitz_regime")]

test <- aut[aut$year==1972,]
