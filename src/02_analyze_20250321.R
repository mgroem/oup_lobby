...
summary(merge$districts)  # 4003 NAs
...
summary(merge$access1)  # 7389 NAs
summary(merge$access2)  # 5500 NAs
...
merge_2020_autocracies <- aut %>%
  filter(year == 2020, v2x_regime2 == "Autocracy")
world <- ne_countries(scale = "medium", returnclass = "sf")
world_autocracies <- left_join(world, merge_2020_autocracies, by = c("iso_a3" = "ISO"))
world_autocracies <- world_autocracies[world_autocracies$continent != "Antarctica", ]
...
m1 <- ggplot() +
  ...
  theme_classic() +
  labs(title = "Interest group institutionalization",caption = "Source: BTI") +
...
m2 <- ggplot() +
  ...
  coord_sf(crs = "+proj=robin") +  # Apply the Robinson projection 
  theme_classic() +
  labs(title = "CSO consultations", caption = "Source: VDem") +
  theme(legend.position = "right",
        panel.background = element_rect(fill = "white", color = NA),
        plot.background = element_rect(fill = "white", color = NA))
