...
vars <- c("access1", "gwf_regimetype4", "undp_hdi", "logpop", "e_regionpol_7C")
...
effect_gwf <- as.data.frame(effect("gwf_regimetype4", m1a)) %>%   # Extract effects a
  mutate(model = "GWF", regime = gwf_regimetype4)

democracy_lines <- effect_gwf %>%  # Get Democracy benchmark
  filter(regime == "Democracy") %>%
  select(model, democracy_fit = fit)

# Plot
Fig18.3 <- ggplot(effect_gwf, aes(x = regime, y = fit, ymin = lower, ymax = upper)) +
  geom_pointrange() +
  geom_hline(
...
ggsave("results/Fig18.3.jpg", plot = Fig18.3, width = 4, height = 4, dpi = 600)
ggsave("results/Fig18.3.pdf", plot = Fig18.3, width = 4, height = 4)
ggsave("results/Fig18.3.eps", plot = Fig18.3, width = 4, height = 4, device = "eps")
...
ggsave("results/Fig18.4.jpg", width = 4, height = 3, dpi = 600)
ggsave("results/Fig18.4.pdf", width = 4, height = 3)
ggsave("results/Fig18.4.eps", width = 4, height = 3, device = "eps")
